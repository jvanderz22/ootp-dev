...
def test():
    ...
    def assert_rounded_exponential(overall, potential, blend, expected, log=False):
        result = model._exponential_interpolation(overall, potential, blend, log)
        rounded = round(result / 5) * 5
        assert (
            rounded == expected
        ), f"Expected {expected}, got {rounded} for overall={overall}, potential={potential}, blend={blend}, reult={result}"

    assert_rounded_exponential(20, 35, BlendScenario.LOW, 20)
    assert_rounded_exponential(20, 40, BlendScenario.LOW, 25)
    assert_rounded_exponential(35, 60, BlendScenario.LOW, 45)
    assert_rounded_exponential(30, 40, BlendScenario.LOW, 30)
    ...
    assert_rounded_exponential(20, 70, BlendScenario.LOW, 35)
    assert_rounded_exponential(40, 60, BlendScenario.LOW, 50)
    assert_rounded_exponential(45, 80, BlendScenario.LOW, 65)
    assert_rounded_exponential(20, 75, BlendScenario.LOW, 40)
    assert_rounded_exponential(20, 80, BlendScenario.LOW, 45)
    ...
    assert_rounded_exponential(20, 60, BlendScenario.LOW, 30)

    assert_rounded_exponential(20, 60, BlendScenario.MEDIUM, 45)
