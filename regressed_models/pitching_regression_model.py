...
class PitchingRegressionModel:
    ...
    def __init__(self, type: str):
        if type == "SP":
            self.pitching_model = StartingPitcherAttributeModel("overall")
        if type == "RP":
            self.pitching_model = ReliefPitcherAttributeModel("overall")
    ...
    def _create_player_scenario(
        ...
        blended_values = []
        for potential_attr, overall_attr in self.PITCHING_ATTRIBUTES:
    ...
    def run(self, player: GamePlayer) -> float:
        ...
        for i, blend_percentage in enumerate(self.BLEND_PERCENTAGES):
            ...
            scenario_score = self.pitching_model.run(scenario_player)
